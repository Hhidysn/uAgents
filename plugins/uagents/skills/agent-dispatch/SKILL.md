...
5. Poll CLI `status <task-id>`, which is local and read-only. Use `result <task-id>` for response, model evidence, usage, captured artifacts, and the requested continuation parent. Call `reconcile <task-id>` only when explicitly checking the stored native identity/execution is appropriate; it never resubmits the original prompt. On Windows, a nonterminal OpenCode Attempt with a durable process can also use `resume <task-id>` to enter that same reconcile path. This is observation recovery, not a new OpenCode turn: only a new submit carrying `session.continue_from_task_id` sends a follow-up prompt to an existing native session. Use the corresponding `uagents_*` tools only on the MCP fallback.
