...
`council-validate` is also local-only. Use either `--validation <file>` or `--profile <name>`. Direct validation JSON uses `schema_version:"1.0"`, legacy `command:[executable,...args]`, or multi-step `checks:[{name,command,timeout_ms?}, ...]` plus optional top-level `timeout_ms` and `on_failure:"continue"|"stop"`. Named profiles live at `<Council source workspace>/.uagents/validation-profiles.json`; each profile body reuses those validation fields without repeating `schema_version`. Profiles are loaded from the source workspace, never candidate worktrees, so every candidate gets the same standard. Checks run in declaration order without a shell; latest evidence records the selected profile plus expanded check outcomes, exit codes, durations and bounded stdout/stderr. A failed validation is evidence for comparison; Council still does not choose the winner automatically.
