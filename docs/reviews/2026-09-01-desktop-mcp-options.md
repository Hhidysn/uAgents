...
采用一个 Plugin + 一个调度 Skill + 两个桌面 MCP。此前只根据官方网页把 `traecli exec`、`mcp-server`、`acp` 列为优先候选，遗漏了本地调研已经验证的关键约束：本需求要消耗已登录 TRAE CN Solo 的免费账号积分；本机 `trae-cn` 没有 headless Agent 返回，独立 `traecli` 是另一产品与额度路线。因此 TRAE 需要复用 TRAECNclaw 的 CDP/网关任务闭环。两个 MCP 是两个服务边界，不是两套重复基础设施。

- TRAE：复用 TRAECNclaw 的 MCP/网关/任务闭环，整理 Windows 补丁、任务归属、取消语义和分发来源；不把官方 `traecli` 当成当前免费积分路线。
...
| 路线 | 证据与适用范围 | 建议 |
| --- | --- | --- |
| TRAE 官方 CLI / MCP | 官方命令文档列出非交互 exec 与 stdio mcp-server，但它是独立产品；本机未安装，也不满足复用 TRAE CN Solo 桌面免费积分的目标 | 排除为本需求执行路线；仅保留为未来不同账号/套餐的独立能力 |
| TRAECNclaw | 本地调研已端到端跑通 MCP→HTTP 网关→CDP→TRAE CN Solo，真实返回回答并消耗桌面账号积分；源代码已有任务 ID、幂等检查、持久化队列和结果回收 | TRAE MCP 的实现基础；须厘清源码来源、提取 Windows 补丁并收紧契约 |
...
TRAE 官方资料中的 CLI 能力本身成立，但不改变本机实测结论：`trae-cn chat` 只投递 GUI 会话而不返回 headless 结果，独立 `traecli` 未安装且不复用目标桌面免费积分。这里按用户的现有额度目标直接排除 CLI，不再把它保留为 TRAE 接入的前置验证项。豆包工作方面，本轮有限的一手资料检索未找到可直接复用的桌面任务入口；这不是不存在的证明。
...
逻辑能力控制在五类；这是拟议契约，不是现成工具名。TRAE 优先保留 TRAECNclaw 已有工具中确有证据的能力，并缩减为清楚的任务接口。
...
1. **TRAE 归档复验与提取。** 以既有真实闭环为基线，在当前版本和准备好的专用窗口复验唯一标识短任务与指定文件产出；不读取凭据，不采用调研文档中的私有 API 路线。
2. **TRAE MCP 收敛。** 从来源明确的 TRAECNclaw 能力中提取或依赖必要部分，带入 Windows fsync 与 Lexical 输入修复；不能直接把脏归档当发行包。
...
本轮 `Get-Command traecli,trae,traecn` 未返回 PATH 可发现的命令；结合本地 2026-08-30 实测，本需求不再等待独立 CLI。下一实现阶段直接处理 TRAECNclaw 的可分发 CDP/MCP 路线，不执行 CLI 登录或安装。
