...
The user must start one dedicated Doubao Work instance with a loopback remote-debugging port. `uagents_probe` confirms the application target without sending a message. Submission obtains shared leases, establishes a blank conversation, persists `possibly_sent` before Enter, and stores the confirmed conversation identity before observation.
