...
Use `target=agy` with an explicit `gemini-*` model. Supported modes are `analysis` and `implementation`. The current verified agy 1.2.5 CLI exposes workspace access through `--add-dir`, but no native file/image attachment flag or other attachment mapping verified by uAgents. Accordingly `inputs.files=false` and `inputs.images=false`; `inputs.workspace_readable=true` describes the separate fact that the agent can read workspace files with its normal tools.

`uagents models agy` runs the native no-prompt `agy models` catalog and caches the validated snapshot in the per-user HostStore for 10 minutes. `uagents models agy --refresh` bypasses that catalog cache. `gemini-3.8-flash-medium` is a verified configured route; other discovered `gemini-*` names are admitted through the existing explicit pattern policy. Models outside that policy can still be shown as discovered but are not automatically allowed.
