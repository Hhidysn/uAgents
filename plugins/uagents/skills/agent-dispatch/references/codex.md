...
V1 supports `analysis` and `implementation` text tasks, workspace access, and native image inputs through `--image <absolute-path>`. Generic file inputs remain unsupported; existing uAgents `expected_outputs` capture files after completion. The adapter does not inject Codex sandbox, bypass, automatic approval, login or credential options. Native Codex permissions remain those of the local Codex configuration; `analysis` is not an enforced filesystem restriction.
...
`probe codex --model gpt-5.6-luna` (or `gpt-6-astra`) only runs `--version` and returns `scope=version_only`, `submission=not_sent`. `models codex` is configured-only until a reliable native no-prompt catalog is available. Images use the registered attachment snapshot and the native CLI image flag; the explicit app-server route uses `localImage`. A real Luna session persisted the exact submitted PNG bytes, although its color answer was incorrect. V1 does not expose generic file attachments, default-route cross-Task `resume/fork`, or durable Codex exec thread reconciliation. `cancel` stops local execution but must not claim that remote generation has been confirmed cancelled. Do not automatically retry a sent/indeterminate task.
