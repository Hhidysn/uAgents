...
test('bundled stdio server initializes without host-injected state variables', async () => {
  fs.mkdirSync(base, { recursive: true }); const home = fs.mkdtempSync(path.join(base, 'trae-home-'));
  const env = { ...process.env, HOME: home, USERPROFILE: home };
  delete env.PLUGIN_DATA; delete env.UAGENTS_STATE_DIR;
  ...
  try {
    ...
    const initialized = await wait(1); assert.equal(initialized.result?.serverInfo?.name, 'uagents-trae-cn');
    assert.equal(fs.statSync(path.join(home, '.uagents', 'trae-cn', 'tasks')).isDirectory(), true);
  } finally {
    child.stdin.end(); await new Promise(resolve => { child.once('close', resolve); setTimeout(() => { child.kill(); resolve(); }, 2000).unref(); }); fs.rmSync(home, { recursive: true, force: true });
  }
});
