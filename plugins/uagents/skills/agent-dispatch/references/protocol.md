...
Unified MCP adds one host-only convenience field without changing the Core schema: `attachments:[{type,local_path,name?}]`. Use it when the chat/connector host has already materialized an upload to a local temporary file. `local_path` must be absolute; `name` is the optional original display filename. The MCP adapter reads the explicit local file and converts it to the existing blob form before `UnifiedRuntime`, so the temporary path is neither part of Core request identity nor persisted Task/Council history. This also means an exact retry may use a different temporary path while preserving the same UUID, provided display name and bytes are unchanged. `inputs` and MCP `attachments` are mutually exclusive. Bytes-only hosts still use the existing `inputs.blob` form.
...
WorkBuddy and OpenCode support explicit continuation and fork contracts. Codex also supports them on the opt-in Windows/Astra app-server route:
...
Exactly one selector may be present. Both forms use a **new** `request_id` and identify a finished previous Task whose persisted native session supplies context. The source Task must use the same target and workspace and must have a persisted native `session_id`. Continuation keeps the same native session; fork requires the target to return a new native session identity. uAgents does not copy old responses into the prompt. WorkBuddy maps continuation to `--resume <session-id>` and fork to `--resume <session-id> --fork-session`; OpenCode maps continuation to `run --session <session-id>` and fork to `run --session <session-id> --fork`. Codex app-server pins the source Thread/Turn, verifies the latest Turn for continuation, and passes `lastTurnId` for fork; both source and follow-up Tasks require Windows, `gpt-6-astra`, and `execution.codex_transport="app-server"`. `status` / `result` expose the requested session selector. CLI/MCP `resume` is different: it recovers or observes the same existing Task/Attempt and never sends a new prompt. `capabilities codex` reports `resume=false` and `fork=false` for the default exec route; `opt_in_transports.app-server` declares the preview's capabilities.

`fallback` must remain `none`. Non-null `max_cost_usd` and unsupported execution timeouts are rejected rather than estimated. On Windows OpenCode, a non-null `execution_timeout_ms` is supported by the durable process path; other targets/platforms still reject it. `analysis` is task intent, not a hard read-only sandbox. `execution.permission` remains accepted and persisted for Schema 1.0 compatibility but is not a uAgents admission gate. Target-native permission behavior follows each Agent's own configuration and supported native arguments. Codex inherits its native configuration; uAgents does not broker its app-server approvals.
...
Input capability is intentionally about **provider-usable native attachment mapping**, not mere filesystem visibility or a local parser accepting a shape. `inputs.workspace_readable` reports whether a target can access the task workspace through its normal tools. `inputs.files` / `inputs.images` are true only when uAgents has target-specific evidence that the mapped input can proceed through the native path. Current agy 1.2.5 exposes no verified native attachment flag/message field, so its file/image flags remain false even though its workspace is readable. Codex CLI v1 sends text over stdin and can use the initialized workspace, but does not yet map its native image flag or file input, so both attachment flags remain false. WorkBuddy 2.132.0 has model-specific image capability: target transport reports `images=true`, `workbuddy-default` tightens it to false because real `auto` E2E rejected image parameters, while the approved `deepseek-v4.1-flash` route keeps `images=true` after a real 512×512 RGB PNG E2E succeeded. WorkBuddy generic files remain false. DeepSeek Harness v1 initializes an absolute workspace through the SDK and therefore reports `workspace_readable=true`, but no native attachment content block has been admitted or provider-verified through uAgents, so `files=false` and `images=false`. OpenCode maps both file and image through repeated native `--file` arguments. uAgents does not fall back to putting unsupported attachment paths into prompt text.
...
- `resume <task-id>`: recovers a `registered/queued` task only when it is unsent, has no native identity/process and has no live worker lease; also resumes first-login waits on the same attempt. If a nonterminal task has a persisted native process (Windows OpenCode or opt-in Codex app-server), `resume` routes to same-Attempt reconcile for `starting`/`running`/`waiting_user`/`indeterminate` instead of dispatch. It never sends the original prompt again or creates another native Turn. A non-durable `waiting_user` task with a native identity is reconciled as before.
