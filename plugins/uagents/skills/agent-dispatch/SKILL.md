...
2. Create one UUID for each intentionally new task and submit the complete versioned request. Reuse the same UUID only for the exact same effective request. Attachment inputs may use workspace-relative `path` or an absolute local `source`; the latter is useful when the host has already materialized a chat/connector file locally. `submit` normalizes `source` into the workspace before registration. To send a deliberate next turn to an already-finished WorkBuddy/OpenCode conversation, still create a new UUID and use exactly one session selector: `session: { continue_from_task_id: "<previous-task-id>" }` continues the same native session, while `session: { fork_from_task_id: "<previous-task-id>" }` creates a new native branch from that context. Do not use the old Task UUID for the new prompt. `submit` auto-prepares the target: it discovers and verifies the local installation and, for desktop targets, starts or reuses the dedicated managed instance. Use `probe` for a read-only check that never starts anything.
