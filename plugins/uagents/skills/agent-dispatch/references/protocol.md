...
Input capability means uAgents has a **native attachment mapping**, not that a workspace file is merely visible or every selected Provider/model has been verified. `inputs.workspace_readable` reports normal tool access to the workspace. Codex maps images through `--image` or app-server `localImage`; generic files remain closed. Claude Code maps images and PDF/UTF-8 text files through stream-json content blocks. DSH maps images through its published SDK inline image block, but its first real image Task ended indeterminate after native process exit, so provider-level success is not confirmed; DSH generic files remain closed. WorkBuddy has model-specific image capability: default/auto rejected image parameters in a real E2E, whereas `deepseek-v4.1-flash` succeeded. WorkBuddy generic files remain closed. OpenCode maps files and images through `--file`. agy, Doubao, and TRAE have no verified native attachment mapping. uAgents does not fall back to putting unsupported attachment paths into prompt text. See `docs/current/attachments.md` and the 2026-09-26 attachment verification record for evidence and limitations.
