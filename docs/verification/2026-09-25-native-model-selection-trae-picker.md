...
- Successful TRAE text + workspace Task response, quota behavior, and actual Provider/model used for generation. Live picker labels and switching to `GLM-5.3` are verified, but the gateway Task failed before `submittedToTrae` and provided no trustworthy per-Task model self-report.
